...
const Wrapper = styled.article`
  ...
  font-family: 'SF Pro JP', 'SF Pro Display', 'SF Pro Icons',
    'Apple WebExp Icons Custom', 'Hiragino Kaku Gothic Pro',
    ...
    <Wrapper id="store">
      <Head>
        ...
        {(ogImage || screenshots[0]) && (
          <meta property="og:image" content={ogImage || screenshots[0]} />
        )}
        ...
        {(ogImage || screenshots[0]) && (
          <meta name="twitter:image:src" content={ogImage || screenshots[0]} />
        )}
      </Head>
      <div className="store-stage">
        <StoreHeader title={title} />
        <div className="content">
          <StoreInformation icon={icon} title={title} author={author} />
          <StoreNote />
          <StoreScreenshot screenshots={screenshots} />
          <StoreDescription description={description} />
          <StoreDownload />
          <StoreUninstall />
          <StoreFooter copyright={copyright} />
        </div>
      </div>
      {isIos && <StoreBalloon />}
      {!isIos && <StoreQr />}
